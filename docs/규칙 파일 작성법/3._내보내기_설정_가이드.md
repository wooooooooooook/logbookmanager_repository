...
Logbook Manager의 CSV/Excel 내보내기 기능은 로그북 데이터를 다양한 형식으로 내보낼 수 있도록 설계되었습니다. 이 기능을 통해 사용자는 데이터를 Excel 파일(.xlsx) 또는 CSV 파일(.csv)로 내보내어 외부 분석 도구에서 활용할 수 있습니다.
...
| `case_sensitive` | boolean       | 옵션      | `false`                                                    | 대소문자 구분 여부 |
...
가장 간단한 방법은 기본 템플릿을 사용하는 것입니다:

```typescript
const exportResult = await exportToCSV({
	template: 'for_record2020',
	format: 'csv'
});
```
...
```typescript
const exportResult = await exportToCSV({
	columns: ['serial_number', 'modality', 'exam_name', 'study_date'],
	format: 'csv'
});
```
...
- CSV 파일의 경우 더 많은 데이터 처리 가능
- 메모리 효율적인 스트리밍 처리

### 3. 메모리 관리

- 스트리밍 방식으로 대용량 파일 처리
- 필요시 데이터를 청크 단위로 분할 처리
